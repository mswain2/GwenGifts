...
function mark_read($id) {
    ...
    $query = "update dbmessages set wasRead=1 where id='$id'";
    $result = mysqli_query($connection, $query);
...
function mark_all_as_read($userID) {
    ...
    $query = "update dbmessages set wasRead=1 where recipientID='$userID'";
    $result = mysqli_query($connection, $query);
...
function message_all_users_of_types($from, $types, $title, $body) {
    ...
    foreach ($rows as $row) {
        $to = mysqli_real_escape_string($connection, $row[0]); // also escape recipient

        $query = "insert into dbmessages 
        (senderID, recipientID, title, body, time, wasRead, prioritylevel)
...
function system_message_all_users_except($except, $title, $body) {
    ...
    $except = mysqli_real_escape_string($connection, $except);
    $title = mysqli_real_escape_string($connection, $title);
    $body = mysqli_real_escape_string($connection, $body);
...
function message_all_users($from, $title, $body) {
    ...
    foreach ($rows as $row) {
        $to = mysqli_real_escape_string($connection, $row[0]);
...
function message_all_users_prio($from, $title, $body, $prio) {
    ...
    $title = mysqli_real_escape_string($connection, $title);
    $body = mysqli_real_escape_string($connection, $body);
    $prio = mysqli_real_escape_string($connection, $prio);
...
function delete_messages_by_ids($ids, $userID) {
    ...
    $query = "DELETE FROM dbmessages 
              WHERE recipientID='$userID' AND id IN ($ids_str)";
